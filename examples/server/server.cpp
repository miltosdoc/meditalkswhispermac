...
void whisper_print_usage(int /*argc*/, char **argv,
  ...
  // server params
  fprintf(stderr,
          "  -dtw MODEL --dtw MODEL         [%-7s] compute token-level "
          "timestamps\n",
          params.dtw.c_str());
  fprintf(stderr,
          "  --host HOST,                   [%-7s] Hostname/ip-adress for the "
          "server\n",
          sparams.hostname.c_str());
  fprintf(
      stderr,
      "  --port PORT,                   [%-7d] Port number for the server\n",
      sparams.port);
  fprintf(stderr,
          "  --public PATH,                 [%-7s] Path to the public folder\n",
          sparams.public_path.c_str());
  fprintf(
      stderr,
      "  --request-path PATH,           [%-7s] Request path for all requests\n",
      sparams.request_path.c_str());
  fprintf(stderr,
          "  --inference-path PATH,         [%-7s] Inference path for all "
          "requests\n",
  ...
  fprintf(stderr,
          "  --tmp-dir,                     [%-7s] Temporary directory for "
          "ffmpeg transcoded files\n",
          sparams.tmp_dir.c_str());
  fprintf(
      stderr,
      "  -sns,      --suppress-nst      [%-7s] suppress non-speech tokens\n",
      params.suppress_nst ? "true" : "false");
  fprintf(stderr,
          "  -nth N,    --no-speech-thold N [%-7.2f] no speech threshold\n",
          params.no_speech_thold);
  fprintf(stderr, "  -ng,       --no-gpu            [%-7s] do not use gpu\n",
          params.use_gpu ? "false" : "true");
  fprintf(
      stderr,
      "  -dev N,    --device N          [%-7d] GPU device ID (default: 0)\n",
      params.gpu_device);
  fprintf(stderr,
          "  -fa,       --flash-attn        [%-7s] enable flash attention\n",
          params.flash_attn ? "true" : "false");
  fprintf(stderr,
          "  -nfa,      --no-flash-attn     [%-7s] disable flash attention\n",
          params.flash_attn ? "false" : "true");
  fprintf(stderr,
          "  -nlp,      --no-language-probabilities [%-7s] exclude language "
          "probabilities from verbose_json output\n",
  ...
  fprintf(
      stderr,
      "  -vm FNAME, --vad-model FNAME               [%-7s] VAD model path\n",
      params.vad_model.c_str());
  fprintf(stderr,
          "  -vt N,     --vad-threshold N               [%-7.2f] VAD threshold "
          "for speech recognition\n",
          params.vad_threshold);
  fprintf(stderr,
          "  -vspd N,   --vad-min-speech-duration-ms  N [%-7d] VAD min speech "
          "duration (0.0-1.0)\n",
          params.vad_min_speech_duration_ms);
  fprintf(stderr,
          "  -vsd N,    --vad-min-silence-duration-ms N [%-7d] VAD min silence "
          "duration (to split segments)\n",
          params.vad_min_silence_duration_ms);
  fprintf(stderr,
          "  -vmsd N,   --vad-max-speech-duration-s   N [%-7s] VAD max speech "
          "duration (auto-split longer)\n",
          params.vad_max_speech_duration_s == FLT_MAX
              ? std::string("FLT_MAX").c_str()
              : std::to_string(params.vad_max_speech_duration_s).c_str());
  fprintf(stderr,
          "  -vp N,     --vad-speech-pad-ms           N [%-7d] VAD speech "
          "padding (extend segments)\n",
          params.vad_speech_pad_ms);
  fprintf(stderr,
          "  -vo N,     --vad-samples-overlap         N [%-7.2f] VAD samples "
          "overlap (seconds between segments)\n",
...
std::string generate_temp_filename(const std::string &path,
  ...
  std::stringstream ss;
  ss << path << std::filesystem::path::preferred_separator << prefix << "-"
...
bool convert_to_wav(const std::string &temp_filename, std::string &error_resp) {
  ...
  cmd_stream << "ffmpeg -nostdin -i \"" << temp_filename
...
void get_req_parameters(const Request &req, whisper_params &params) {
  ...
  if (req.has_file("entropy_thold")) {
    params.entropy_thold =
        std::stof(req.get_file_value("entropy_thold").content);
  }
  if (req.has_file("logprob_thold")) {
    params.logprob_thold =
        std::stof(req.get_file_value("logprob_thold").content);
  }
  if (req.has_file("debug_mode")) {
    params.debug_mode =
        parse_str_to_bool(req.get_file_value("debug_mode").content);
  }
  if (req.has_file("translate")) {
    params.translate =
        parse_str_to_bool(req.get_file_value("translate").content);
  }
  ...
  if (req.has_file("tinydiarize")) {
    params.tinydiarize =
        parse_str_to_bool(req.get_file_value("tinydiarize").content);
  }
  if (req.has_file("split_on_word")) {
    params.split_on_word =
        parse_str_to_bool(req.get_file_value("split_on_word").content);
  }
  if (req.has_file("no_timestamps")) {
    params.no_timestamps =
        parse_str_to_bool(req.get_file_value("no_timestamps").content);
  }
  ...
  if (req.has_file("detect_language")) {
    params.detect_language =
        parse_str_to_bool(req.get_file_value("detect_language").content);
  }
  ...
  if (req.has_file("temperature_inc")) {
    params.temperature_inc =
        std::stof(req.get_file_value("temperature_inc").content);
  }
  if (req.has_file("suppress_non_speech")) {
    params.suppress_nst =
        parse_str_to_bool(req.get_file_value("suppress_non_speech").content);
  }
  if (req.has_file("suppress_nst")) {
    params.suppress_nst =
        parse_str_to_bool(req.get_file_value("suppress_nst").content);
  }
  if (req.has_file("vad")) {
  ...
  if (req.has_file("vad_threshold")) {
    params.vad_threshold =
        std::stof(req.get_file_value("vad_threshold").content);
  }
  if (req.has_file("vad_min_speech_duration_ms")) {
    params.vad_min_speech_duration_ms =
        std::stof(req.get_file_value("vad_min_speech_duration_ms").content);
  }
  if (req.has_file("vad_min_silence_duration_ms")) {
    params.vad_min_silence_duration_ms =
        std::stof(req.get_file_value("vad_min_silence_duration_ms").content);
  }
  if (req.has_file("vad_max_speech_duration_s")) {
    params.vad_max_speech_duration_s =
        std::stof(req.get_file_value("vad_max_speech_duration_s").content);
  }
  if (req.has_file("vad_speech_pad_ms")) {
    params.vad_speech_pad_ms =
        std::stoi(req.get_file_value("vad_speech_pad_ms").content);
  }
  if (req.has_file("vad_samples_overlap")) {
    params.vad_samples_overlap =
        std::stof(req.get_file_value("vad_samples_overlap").content);
  }
  if (req.has_file("no_language_probabilities")) {
    params.no_language_probabilities = parse_str_to_bool(
        req.get_file_value("no_language_probabilities").content);
  }
}
...
int main(int argc, char **argv) {
  ...
  std::string const default_content =
    ...
    <body>
        ...
        <h2>)" +
      sparams.request_path + sparams.inference_path + R"(</h2>
        <pre>
    curl 127.0.0.1:)" +
      std::to_string(sparams.port) + sparams.request_path +
      sparams.inference_path + R"( \
    ...
    -F response_format="json"
        ...
        <pre>
    curl 127.0.0.1:)" +
      std::to_string(sparams.port) + R"(/load \
    -H "Content-Type: multipart/form-data" \
    -F model="&lt;path-to-model-file&gt;"
        ...
        <div>
            <h2>Try it out</h2>
            <form action=")" +
      sparams.request_path + sparams.inference_path +
  ...
  svr->Post(sparams.request_path + sparams.inference_path, [&](const Request
                                                                   &req,
                                                               Response &res) {
    ...
    if (sparams.ffmpeg_converter) {
      ...
      const std::string temp_filename =
          generate_temp_filename(sparams.tmp_dir, "whisper-server", ".wav");
    ...
    // run the inference
    {
      ...
      wparams.token_timestamps =
          !params.no_timestamps && params.response_format == vjson_format;
      ...
      wparams.vad_params.min_speech_duration_ms =
          params.vad_min_speech_duration_ms;
      wparams.vad_params.min_silence_duration_ms =
          params.vad_min_silence_duration_ms;
      wparams.vad_params.max_speech_duration_s =
          params.vad_max_speech_duration_s;
      ...
      if (whisper_full_parallel(ctx, wparams, pcmf32.data(), pcmf32.size(),
        ...
        if (req.is_connection_closed()) {
          ...
          res.set_content("{\"error\":\"client disconnected\"}",
                          "application/json");
          return;
        }
    ...
    if (params.response_format == text_format) {
      ...
      std::string results = output_str(ctx, params, pcmf32s);
      json jres =
          json{{"task", params.translate ? "translate" : "transcribe"},
      ...
      if (!params.no_language_probabilities) {
        ...
        const auto detected_lang_id = whisper_lang_auto_detect(
            ctx, 0, params.n_threads, lang_probs.data());
        ...
        for (int i = 0; i <= whisper_lang_max_id(); ++i) {
          if (lang_probs[i] >
              0.001f) { // Only include non-negligible probabilities
  ...
  svr->Post(sparams.request_path + "/load", [&](const Request &req,
                                                Response &res) {
    std::lock_guard<std::mutex> lock(whisper_mutex);
    ...
    if (!req.has_file("model")) {
      ...
      const std::string error_resp =
          "{\"error\":\"no 'model' field in the request\"}";
      res.set_content(error_resp, "application/json");
      return;
    }
  ...
  svr->Get(
      sparams.request_path + "/health", [&](const Request &, Response &res) {
